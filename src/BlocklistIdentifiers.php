...
class BlocklistIdentifiers
{
    ...
    public function create(?Operations\CreateBlocklistIdentifierRequestBody $request = null, ?Options $options = null): Operations\CreateBlocklistIdentifierResponse
    {
        ...
        $hookContext = new HookContext($baseUrl, 'CreateBlocklistIdentifier', [], $this->sdkConfiguration->securitySource);
    ...
    public function delete(string $identifierId, ?Options $options = null): Operations\DeleteBlocklistIdentifierResponse
    {
        ...
        $hookContext = new HookContext($baseUrl, 'DeleteBlocklistIdentifier', [], $this->sdkConfiguration->securitySource);
        $httpRequest = $this->sdkConfiguration->hooks->beforeRequest(new Hooks\BeforeRequestContext($hookContext), $httpRequest);
        $httpOptions = Utils\Utils::convertHeadersToOptions($httpRequest, $httpOptions);
        $httpRequest = Utils\Utils::removeHeaders($httpRequest);
        try {
    ...
    public function list(?Options $options = null): Operations\ListBlocklistIdentifiersResponse
    {
        ...
        $hookContext = new HookContext($baseUrl, 'ListBlocklistIdentifiers', [], $this->sdkConfiguration->securitySource);
        $httpRequest = $this->sdkConfiguration->hooks->beforeRequest(new Hooks\BeforeRequestContext($hookContext), $httpRequest);
        $httpOptions = Utils\Utils::convertHeadersToOptions($httpRequest, $httpOptions);
        $httpRequest = Utils\Utils::removeHeaders($httpRequest);
        try {
        ...
        if (Utils\Utils::matchStatusCodes($statusCode, ['200'])) {
            if (Utils\Utils::matchContentType($contentType, 'application/json')) {
                ...
                $serializer = Utils\JSON::createSerializer();
                $responseData = (string) $httpResponse->getBody();
                $obj = $serializer->deserialize($responseData, '\Clerk\Backend\Models\Components\BlocklistIdentifiers', 'json', DeserializationContext::create()->setRequireAllRequiredProperties(true));
